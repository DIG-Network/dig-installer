...
`schedule` trigger — §8.1) selecting `channel: stable` or `channel: both`, and only when the
version in the root `Cargo.toml` (`[package].version`) has advanced beyond the newest `vX.Y.Z` tag
